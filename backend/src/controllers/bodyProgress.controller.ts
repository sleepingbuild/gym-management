...
import { createBodyProgressSchema, updateBodyProgressSchema } from '../validators/bodyProgress.validator';
  ...
  async getLatest(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(200).json({
        success: true,
        statusCode: 200,
        message: record ? 'Latest progress record retrieved' : 'No progress records found',
        data: record,
      });
    } catch (err) {
      next(err);
