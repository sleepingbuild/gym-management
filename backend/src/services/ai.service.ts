...
async function retrieveContext(query: string): Promise<string> {
  ...
  return docs.map((d, i) => `[${i + 1}] ${d.title}: ${d.content}`).join('\n\n');
}
...
async function checkAndUpdateUsage(userId: string): Promise<void> {
  ...
  if (!membership) {
    throw new AppError(403, 'AI_001: No active membership. Please purchase a plan.');
  }

  const now = new Date();
  const isNewDay = now.toDateString() !== new Date(membership.aiDailyReset).toDateString();
  const isNewMonth =
  ...
  if (aiDailyLimit !== -1 && aiDailyCount >= aiDailyLimit) {
    throw new AppError(429, `AI_002: Daily limit reached (${aiDailyLimit}/day). Upgrade your plan.`);
  }
  if (aiLimit !== -1 && aiUsageCount >= aiLimit) {
    throw new AppError(429, `AI_003: Monthly limit reached (${aiLimit}/month). Upgrade your plan.`);
  }
...
- Cau tra loi ngan gon, de hieu, co the dung emoji phu hop`;
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [
      ...
      { role: 'model', parts: [{ text: 'Toi hieu, toi se ho tro ban voi tu cach la AI Personal Trainer cua IronFit Pro.' }] },
      ...conversationHistory,
  ...
  await prisma.chatHistory.createMany({
    data: [
      ...
      { userId, sessionId: currentSessionId, role: 'assistant', content: answer },
    ],
  });
...
async function getChatHistory(userId: string, sessionId?: string) {
  ...
  return prisma.chatHistory.findMany({ where, orderBy: { createdAt: 'asc' } });
}
