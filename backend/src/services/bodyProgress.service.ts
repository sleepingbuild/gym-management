...
import { CreateBodyProgressDTO, UpdateBodyProgressDTO } from '../types/bodyProgress.types';
...
export const bodyProgressService = {
  ...
  async create(userId: string, dto: CreateBodyProgressDTO) {
    ...
    if (!membership || membership.status !== 'ACTIVE') {
      throw new AppError(403, 'Active membership required to track progress', 'MEMBERSHIP_005');
    }
  ...
  async update(id: string, userId: string, dto: UpdateBodyProgressDTO) {
    ...
    if (!existing) {
      throw new AppError(404, 'Progress record not found', 'PROGRESS_001');
    }
  ...
  async getStats(userId: string) {
    ...
    return {
      ...
      progress: {
        ...
        bmi: last.bmi && first.bmi ? Math.round((last.bmi - first.bmi) * 10) / 10 : null,
        bodyFat: last.bodyFat && first.bodyFat ? Math.round((last.bodyFat - first.bodyFat) * 10) / 10 : null,
      },
      average: {
        weight: Math.round((records.reduce((sum, r) => sum + r.weight, 0) / records.length) * 10) / 10,
        bmi: records.filter(r => r.bmi).length > 0 
          ? Math.round((records.reduce((sum, r) => sum + (r.bmi || 0), 0) / records.filter(r => r.bmi).length) * 10) / 10 
          : null,
      },
    };
  },
