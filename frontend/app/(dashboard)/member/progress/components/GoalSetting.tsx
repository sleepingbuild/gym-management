...
import { bodyGoalService, BodyGoal, CreateGoalDTO } from '@/services/bodyGoal.service';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, targetWeight: parseFloat(e.target.value) })
              }
            />
            <Input
              label="BMI mục tiêu"
              ...
              onChange={(e) =>
                setFormData({ ...formData, targetBmi: parseFloat(e.target.value) })
              }
            />
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, targetBodyFat: parseFloat(e.target.value) })
              }
            />
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, targetMuscle: parseFloat(e.target.value) })
              }
            />
          </div>
          <Input
  ...
  return (
    <Card>
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-title-md font-display text-ink">🎯 Mục tiêu hiện tại</h3>
          <p className="text-body-sm text-muted">
            {goal.status === 'ACTIVE' ? 'Đang theo dõi' : goal.status === 'ACHIEVED' ? '✅ Đã đạt!' : '⏳ Đã hết hạn'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
        {goal.targetWeight && (
          <div>
            <p className="text-caption uppercase text-muted">Cân nặng mục tiêu</p>
            <p className="text-title-md font-display text-ink">{goal.targetWeight} kg</p>
          </div>
        )}
        {goal.targetBmi && (
          <div>
            <p className="text-caption uppercase text-muted">BMI mục tiêu</p>
            <p className="text-title-md font-display text-ink">{goal.targetBmi}</p>
          </div>
        )}
        {goal.targetBodyFat && (
          <div>
            <p className="text-caption uppercase text-muted">Mỡ mục tiêu</p>
            <p className="text-title-md font-display text-ink">{goal.targetBodyFat}%</p>
          </div>
        )}
        {goal.targetMuscle && (
          <div>
            <p className="text-caption uppercase text-muted">Cơ mục tiêu</p>
            <p className="text-title-md font-display text-ink">{goal.targetMuscle} kg</p>
          </div>
        )}
      </div>
      <p className="text-body-sm text-muted mt-4">
        Ngày dự kiến đạt: {new Date(goal.targetDate).toLocaleDateString('vi-VN')}
      </p>
      {goal.notes && (
